...
バゲッジと片仮名で書くとよくわからないので、Baggage と書きます。  
Baggage はスパン間を伝搬するコンテキスト情報です。
複数のサービス間で Baggage を伝搬することで、分散システムにおけるトレーサビリティを実現します。  

例えば、`CustomerId` という属性を複数のサービスで構成されたトレースで使い要望があるとします。ただ、`CustomerId` はその中の1つのサービスでしか有効ではありません。
ここで Baggage を使ってトレース全体に `CustomerId` を伝搬します。Baggage は Key-Value 形式でストアされます。サービスで取得した値を `CustomerId=123` のように伝搬します。  


## セキュリティ上の留意

Baggage にストアした値は意図しないネットワークに伝搬される可能性があることを留意するべきです。  
Baggage は HTTP ヘッダーで送信されます。完全にコントールされたネットワーク内のみでシグナルが伝搬されるならまだしも、そうでない場合はリスクがあります。  
